...
2. For every hashtag in the list, it will open the page and click the first picture to open it
3. It will then like, follow, comment and move to the next picture, in a 200 iterations loop (number can be adjusted)
4. Saves a list with all users you followed using the bot
'''

from selenium import webdriver
from selenium.webdriver.common.keys import Keys # (could be lowercase keys)
from time import sleep, strftime
from random import randint
import pandas as pandas

chromedriver_path = '/Users/ralphramos/Documents/chromedriver 2'
...
username.send_keys('rlramos@ucdavis.edu')
password = webdriver.find_element_by_name('password')
password.send_keys('2LMG@bbSnKUg*%2')
...
hashtag_list = ['funny'] # enter hashtag(s)
...
for hashtag in hashtag_list:
    ...
    sleep(randint(1,2))
    try:
        for x in range(0,3): # Enter the number of iterations
            username = webdriver.find_element_by_xpath('/html/body/div[5]/div[2]/div/article/header/div[2]/div[1]/div[1]/a').text
            ...
            if username not in prev_user_list:
                ...
                if webdriver.find_element_by_xpath('/html/body/div[5]/div[2]/div/article/header/div[2]/div[1]/div[2]/button').text == 'Follow':
                    ...
                    # Liking the picture
                    button_like = webdriver.find_element_by_xpath('/html/body/div[5]/div[2]/div/article/div[2]/section[1]/span[1]/button/svg')
                    ...
                    comm_prob = randint(7,10) # choose probability (1,10)
                    print(f'{hashtag}_{x}: {comm_prob}') #iteration log message
                    if comm_prob: # > 7
                        comments += 1
                        ...
                        if comm_prob == 7:
                            ...
                            comment_box.send_keys('Funny, that is hilarious!')
                        elif comm_prob == 10:
                            comment_box.send_keys('So cool! xD')
                            sleep(1)
...
updated_user_df = pandas.DataFrame(prev_user_list)
